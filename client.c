...
void stampaMappa(char A[][20],int,int,int,int);
void gioco(int sock);
...
{	
	...
	mappa[i][j]=coordinate[2];
		...
		if(read(sock,avversari,strlen(avversari))<0){
			error("Errore write\n");	
		}
	...
	do{	
		...
		system("clear");
		stampaMappa(mappa,i,j,id_corrente,sock);
...
void stampaMappa(char mappa[][20],int pos1, int pos2,int id_corrente,int sock)
{
	int i=0, j=0;
	char messaggio[256];
	char flag[256];
