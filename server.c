...
// FAI QUESTO = POSIZIONAMI
	...
	//inviare qui l'id partita al client
...
//________________________________________________________________________STRINGA AVVERSARI_________________________________________________________________________
...
void prova(struct utente* Ut){

/*__________ stampa utenti & coordinate______*/
		...
		while (tmp !=NULL){
			...
			printf("1) entrato nel while \n");
			printf("u : %s\n",tmp->nick);
			

			printf("2) stampato utente \n");

			//stampa nel server
			//printf("nick: %s \nx: %d \ny: %d \n", tmp->nick, tmp->x, tmp->y);
			...
			printf("4) salvato nella stringa \n");
			
			int w1=w-2;

			printf("%s 5) stampo stringa : \n",stringa);

			
			int x2,y2;

			//for(i=0;i<strlen(stringa);i+=2){
